...
void biosBoot(String status){
  ...
  if(biosLine==10){
    ...
    tft.drawString("WRAITH UEFI BIOS v3.10", 6, 4);

    biosLine = 28;

    // Sistem bilgileri - daha gerçekçi format
    tft.setTextColor(TFT_WHITE);
    tft.drawString("Processor ........ ESP8266EX @80MHz", 10, biosLine); biosLine+=14;
    tft.drawString("Memory Test ...... 8192 KB  [OK]", 10, biosLine); biosLine+=14;
    tft.drawString("Video Adapter .... TFT_eSPI", 10, biosLine); biosLine+=14;
    tft.drawString("Firmware Version . "+currentVersion, 10, biosLine); biosLine+=14;
    tft.drawString("Boot Services .... Initializing", 10, biosLine); biosLine+=18;
...
void setup(){
  ...
  biosBoot(WiFi.status()==WL_CONNECTED ? "WiFi [OK]" : "WiFi [FAILED]"); delay(600);
...
void loop(){
  ...
  if(millis() - lastUpdateCheck > UPDATE_INTERVAL){
    ...
    // OTA kontrolü sirasinda LED kirmizi
    setLedUpdate();
    checkForUpdate();
    // OTA basarisiz ya da guncel -> tekrar normal moda don
    setLedNormal();
  }
